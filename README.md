...
- **Draw on the whole page** — tap the pen to drop into **freehand ink mode** and write or annotate with a finger or stylus **anywhere over the note**, mixing handwriting with your typed text, images and tables (not confined to a box). Pick a pen colour and thickness; two‑finger drag scrolls the page while you draw. **Convert your handwriting to typed text** with one tap (on‑device). By default, when you finish a drawing your **typed text continues on a fresh line below it** (so text never lands on your strokes) — switch this to a **free overlay** in **Settings → Additional configurations → Drawing & text**. The ink scrolls and saves with the note, and syncs and exports along with it.
